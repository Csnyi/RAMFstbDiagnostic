...
- Add dynamic content
  */
  ...
  var content = {
  manualText: [
  `R.A.M.F. Diagnostic is a Chrome browser extension for monitoring and diagnosing the Goldmaster SR-525HD satellite set-top box over a local network.  Once installed, enter the device's IP address in the connection field and press the Connection button.  The extension will establish communication with the device and unlock the available diagnostic features: Spectrum Report and SNR Measurement.`,
  ...
  imgDescript: [
  `The SR-525HD is a Goldmaster DVB-S2 satellite receiver with a built-in HTTP API,  allowing network-based diagnostics from any device on the same local network.  It supports DiSEqC 1.0 and 1.1, multi-satellite reception, and provides real-time signal parameters  including SNR, LNB current and voltage, lock status, and carrier offset.`,
